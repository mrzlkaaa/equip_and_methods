...
class AnylizePeaks:
    ...
    def msd(self):  # * average background noise value
        msd = math.sqrt(
            sum([(i-self.y_mean)**2 for i in self.y_data])/len(self.y_data))
    ...
    def linear_regression(self, x, y):
        # * reshape to [[val1], [val2], ... [valn]]
        resh_x = x.reshape((-1, 1))
...
# plt.plot(noise_x, linear_y_noise, "black", label="linear fit of noise")
chi_linear = ap.get_chi(noise_y, linear_y_noise)
print("chi linear - ", chi_linear)
...
# plt.plot(signal_x, y_gauss_true_signal, "green", label="gause fit of signal")
chi_gauss = ap.get_chi(signal_y, y_gauss_true_signal)
print("lengths of gausse/linear arrays",
      len(y_gauss_signal), len(linear_y_noise))
print("chi gause - ", chi_gauss)
print(f"S/N raio is {2*signal_y.max()/noise_y.mean()}")
